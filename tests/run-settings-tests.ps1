...
$app = Join-Path $project '便携版-0.3.3\拆包助手.exe'
...
foreach ($name in @('ConfigurationTests','PreferencesUiTests','PasswordPersistenceTests')) {
    & $compiler /nologo /target:exe /platform:x64 /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.Security.dll ('/reference:'+$app) ('/out:'+(Join-Path $runtime ($name+'.exe'))) (Join-Path $PSScriptRoot ($name+'.cs'))
